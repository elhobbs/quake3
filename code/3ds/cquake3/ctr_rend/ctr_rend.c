...
void ctr_set_attr_buffers(int element_count) {
	...
	if (bound_array_buffer) {
		...
		buff_offs[0] = CTR_RENDER_OFFSET(buf->data);
	...
	myGPU_SetAttributeBuffers(
		...
		(u32*)osConvertVirtToPhys((u32)__linear_heap), // Location of the VBO
...
void glDrawElementsBaseVertex(GLenum mode, GLsizei count, GLenum type, const GLvoid *indices, GLint basevertex) {
	...
	GPU_SetFloatUniform(GPU_VERTEX_SHADER, uLoc_modelView, (u32*)mat, 4);

	mmode = GL_PROJECTION;
	depth = ctr_state.matrix_depth[mmode];
	mat = &ctr_state.matrix[mmode][depth];
	GPU_SetFloatUniform(GPU_VERTEX_SHADER, uLoc_projection, (u32*)mat, 4);
