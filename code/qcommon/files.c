	...
	*file = FS_HandleForFile();
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		if ( search->pack && search->pack->hashTable[hash] ) {
			...
			pak = search->pack;
			pakFile = pak->hashTable[hash];
			waitforit(pak->pakFilename);
			do {
				// case and separator insensitive comparisons
				if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
					// found it!
					waitforit(pakFile->name);
					...
					waitforit("FS_FOpenFileRead 1a");
					Q_strncpyz(fsh[*file].name, filename, sizeof(fsh[*file].name));
					fsh[*file].zipFile = qtrue;
					zfi = (unz_s *)fsh[*file].handleFiles.file.z;
					// in case the file was new
					temp = zfi->file;
					waitforit("FS_FOpenFileRead 1b");
					// set the file position in the zip file (also sets the current file info)
					unzSetCurrentFileInfoPosition(pak->handle, pakFile->pos);
					// copy the file info into the unzip structure
					waitforit("FS_FOpenFileRead 1c");
					...
					//printf("%d\n", zfi->cur_file_info.uncompressed_size);
					waitforit("FS_FOpenFileRead 1d");
...
int FS_ReadFile( const char *qpath, void **buffer ) {
	...
	waitforit("FS_ReadFile 1");
	if (h == 0) {
		waitforit("FS_ReadFile 1a");
		if (buffer) {
			*buffer = NULL;
		}
	...
	waitforit("FS_ReadFile 2");
	buf = Hunk_AllocateTempMemory(len+1);
	*buffer = buf;

	//printf("%08x\n", buf);
	waitforit("FS_ReadFile 3");
	FS_Read(buf, len, h);

	// guarantee that it will have a trailing 0 for string operations
	buf[len] = 0;
	FS_FCloseFile( h );
	waitforit("FS_ReadFile 4");
	...
	for (search = fs_searchpaths ; search ; search = search->next) {
		...
		if (search->pack) {
			...
			if ( fs_restrict->integer || fs_numServerPaks ) {
				...
				printf("path2: %s\n", netpath);
				waitforit("Sys_ListFiles before");
				sysFiles = Sys_ListFiles(netpath, extension, filter, &numSysFiles, qfalse);
				waitforit("Sys_ListFiles after");
