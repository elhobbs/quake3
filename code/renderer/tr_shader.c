...
static void ScanAndLoadShaderFiles( void )
{
	...
	int shader_table_size = sizeof(int)*MAX_SHADERTEXT_HASH;
	int hash, size;

	long sum = 0;

	waitforit("ScanAndLoadShaderFiles 1");
	...
	waitforit("ScanAndLoadShaderFiles 2");
	buffers = (char **)malloc(sizeof(char*)*MAX_SHADER_FILES);
	shaderTextHashTableSizes = (int *)malloc(shader_table_size);
	if (buffers == 0 || shaderTextHashTableSizes == 0) {
		ri.Error(ERR_FATAL, "Couldn't alloc buffers in ScanAndLoadShaderFiles");
	}
	...
	waitforit("ScanAndLoadShaderFiles 3");
	...
	waitforit("ScanAndLoadShaderFiles 4");
	...
	waitforit("ScanAndLoadShaderFiles 5");
	// free up memory
	ri.FS_FreeFileList( shaderFiles );

	waitforit("ScanAndLoadShaderFiles 6");
	Com_Memset(shaderTextHashTableSizes, 0, shader_table_size);
	size = 0;
	//
	for ( i = 0; i < numShaders; i++ ) {
	...
	waitforit("ScanAndLoadShaderFiles 7");
	...
	waitforit("ScanAndLoadShaderFiles 8");
	Com_Memset(shaderTextHashTableSizes, 0, shader_table_size);
	//
	for ( i = 0; i < numShaders; i++ ) {
		// pointer to the first shader file
	...
	waitforit("ScanAndLoadShaderFiles 9");
	free(buffers);
	free(shaderTextHashTableSizes);

	waitforit("ScanAndLoadShaderFiles A");
	return;
...
static void CreateExternalShaders( void ) {
	waitforit("CreateExternalShaders 1");
	tr.projectionShadowShader = R_FindShader( "projectionShadow", LIGHTMAP_NONE, qtrue );
	waitforit("CreateExternalShaders 2");
	tr.flareShader = R_FindShader("flareShader", LIGHTMAP_NONE, qtrue);
	waitforit("CreateExternalShaders 3");
	tr.sunShader = R_FindShader("sun", LIGHTMAP_NONE, qtrue);
	waitforit("CreateExternalShaders 4");
}
...
void R_InitShaders( void ) {
	...
	waitforit("R_InitShaders 1");
	CreateInternalShaders();

	waitforit("R_InitShaders 2");
	ScanAndLoadShaderFiles();

	waitforit("R_InitShaders 3");
	CreateExternalShaders();
	waitforit("R_InitShaders 4");
}
