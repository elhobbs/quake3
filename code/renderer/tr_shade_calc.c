...
void RB_CalcDeformVertexes( deformStage_t *ds )
{
	...
	if ( ds->deformationWave.frequency == 0 )
	{
		...
		for ( i = 0; i < tess.numVertexes; i++, xyz += 3, normal += 3)
		{
...
void RB_CalcBulgeVertexes( deformStage_t *ds ) {
	...
	for ( i = 0; i < tess.numVertexes; i++, xyz += 3, st += 4, normal += 3) {
		int		off;
		float scale;
...
static void AutospriteDeform( void ) {
	...
	for ( i = 0 ; i < oldVerts ; i+=4 ) {
		...
		mid[0] = 0.25f * (xyz[0] + xyz[3] + xyz[6] + xyz[9]);
		mid[1] = 0.25f * (xyz[1] + xyz[4] + xyz[7] + xyz[10]);
		mid[2] = 0.25f * (xyz[2] + xyz[5] + xyz[8] + xyz[11]);
...
void RB_CalcEnvironmentTexCoords( float *st ) 
{
	...
	for (i = 0 ; i < tess.numVertexes ; i++, v += 3, normal += 3, st += 2 ) 
	{
...
void RB_CalcSpecularAlpha( unsigned char *alphas ) {
	...
	for (i = 0 ; i < numVertexes ; i++, v += 3, normal += 3, alphas += 4) {
		float ilength;
...
#endif
	numVertexes = tess.numVertexes;
	for (i = 0 ; i < numVertexes ; i++, v += 3, normal += 3) {
#if idppc_altivec
