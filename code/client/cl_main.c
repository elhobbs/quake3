...
void CL_InitRenderer( void ) {
	waitforit("CL_InitRenderer 1");
	// this sets up the renderer and calls R_Init
	re.BeginRegistration( &cls.glconfig );

	// load character sets
	waitforit("CL_InitRenderer 2");
	cls.charSetShader = re.RegisterShader("gfx/2d/bigchars");
	waitforit("CL_InitRenderer 3");
	cls.whiteShader = re.RegisterShader("white");
	waitforit("CL_InitRenderer 4");
	cls.consoleShader = re.RegisterShader("console");
	waitforit("CL_InitRenderer 5");
	g_console_field_width = cls.glconfig.vidWidth / SMALLCHAR_WIDTH - 2;
	g_consoleField.widthInChars = g_console_field_width;
	waitforit("CL_InitRenderer 6");
}
...
void CL_StartHunkUsers( void ) {
	...
	waitforit("CL_StartHunkUsers 1");
	if ( !cls.rendererStarted ) {
		cls.rendererStarted = qtrue;
		CL_InitRenderer();
	}

	waitforit("CL_StartHunkUsers 2");
	if (!cls.soundStarted) {
		cls.soundStarted = qtrue;
		S_Init();
	}

	waitforit("CL_StartHunkUsers 3");
	if (!cls.soundRegistered) {
		cls.soundRegistered = qtrue;
		S_BeginRegistration();
	}

	waitforit("CL_StartHunkUsers 4");
	if (!cls.uiStarted) {
		cls.uiStarted = qtrue;
		CL_InitUI();
	}
	waitforit("CL_StartHunkUsers 5");
}
