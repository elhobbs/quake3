...
static int CIN_Initialized = 0;
static byte *CIN_linbuf = 0;// DEFAULT_CIN_WIDTH*DEFAULT_CIN_HEIGHT * 4 * 2
/*
==================
CL_PlayCinematic
...
int CIN_PlayCinematic( const char *arg, int x, int y, int w, int h, int systemBits ) {
	...
	if (CIN_linbuf == 0) {
		CIN_linbuf = (byte *)malloc(DEFAULT_CIN_WIDTH*DEFAULT_CIN_HEIGHT * 4 * 2);
		if (CIN_linbuf == 0) {
			printf("CIN_linbuf == 0\n");
			while (1);
		}
